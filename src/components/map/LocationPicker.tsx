...
export default function LocationPicker() {
  ...
  const fetchAddress = useCallback(async (lat: number, lng: number, fallbackAddress = '') => {
    ...
    try {
      ...
      if (res && res.length > 0) {
        ...
        setAddress(parts.join(' ') || fallbackAddress);
      } else {
        setAddress(fallbackAddress);
      }
    } catch {
      if (seq === geocodeSeqRef.current) setAddress(fallbackAddress);
    } finally {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (result.length > 0) {
        ...
        const nextCoord: [number, number] = [longitude, latitude];

        centerRef.current = { lat: latitude, lng: longitude };
        lastUpdateCoordRef.current = centerRef.current;
        setCenterCoord(nextCoord);
        setIsMapMoving(false);
        updateBoundedState(latitude, longitude);
        setAddress(query);
        ...
        void fetchAddress(latitude, longitude, query);
      } else {
