...
export default function MediaPickerModal({
  ...
  }, [clearMediaRefreshTimers, ty, onClose]);
  ...
  }, [clearMediaRefreshTimers, openSheet, visible]);
  ...
  const initPermissionAndLoad = async () => {
    ...
    try {
      ...
      if (permissionRequestSeqRef.current !== requestSeq || !visibleRef.current || !isMounted.current) return;
      ...
      if (permissionRequestSeqRef.current !== requestSeq || !visibleRef.current || !isMounted.current || !mediaPermissionGrantedRef.current) return;
  ...
  const refreshVisibleMediaLibrary = useCallback(async () => {
    ...
    try {
      ...
      const nextCurrentAlbum = activeAlbum
        ? visibleAlbums.find((album) => album.id === activeAlbum.id) ?? null
        : null;
  ...
  }, [clearMediaRefreshTimers, mediaPermissionState, scheduleMediaLibraryRefresh, visible]);
