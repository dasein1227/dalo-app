...
export async function sendRoomMessage(args: SendRoomMessageArgs) {
  ...
  await database.write(async () => {
    ...
    await collection.create((m: any) => {
      ...
      safeSet(m, 'sender_selected_tier', computed.senderSelectedTier ?? 'free');
      safeSet(m, 'max_generated_tier', computed.maxGeneratedTier ?? (computed.senderSelectedTier ?? 'free'));
  ...
  // ✅ tier 값은 서버가 null 가정하면 바로 죽기 쉬우니 "항상" 보낸다 (서버 안전성)
  payload.source_lang = computed.sourceLang ?? null;
  payload.sender_selected_tier = computed.senderSelectedTier ?? 'free';
  ...
  if (fnErr) {
    ...
    // ✅ DEV에서는 무조건 raw fetch로 "진짜" 에러 바디 확보 (DEBUG_CHAT 조건 제거)
    if (__DEV__) {
      try {
  ...
  if (!fnRes?.success || !fnRes?.message) {
    ...
    if (DEBUG_CHAT) console.warn('[push.ts] message-send bad response detail:', fnRes);
    return;
  }
  ...
  // - realtime INSERT가 이미 들어와 local_가 삭제된 경우에도 안전
  // - local_ 중복 제거 + 서버 메시지 upsert를 한 번의 batch로 처리
  try {
    await upsertMessagesAtomic([fnRes.message as ChatMessageRow]);
  } catch (e: any) {
    // 최후의 안전망(이 경로는 보통 타지 않아야 함)
