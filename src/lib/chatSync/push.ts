...
/**
 * 정책:
 * - free 회원은 preferred_lang(보낼 언어)을 사용하지 않는다(클라 조작/구버전 방지).
 * - DM(direct) 타깃 언어는 상대 view_lang 우선으로 고정된다.
 */
function pickTargetLang(args: SendRoomMessageArgs, assumedRoomType: RoomType): string | null {
  ...
  const mySend =
    senderTier === 'free'
      ? null
      : normalizeLangCode(args.my?.preferred_lang ?? null);
...
/**
 * invoke 에러 시 DEV에서 raw fetch로 에러 바디 확보.
 */
...
async function computeContentIfNeeded(args: SendRoomMessageArgs): Promise<{
  ...
  try {
    const raw = await args.translateFn({
      ...
      tier: finalTier,
      tone:
        assumedRoomType === 'self' && finalTier === 'high'
          ? args.my?.translation_tone_default ?? null
          : null,
      sourceLang: args.sourceLang ?? null,
    });
...
/**
 * "나에게만 삭제": 서버 tombstone 기록 + 로컬 물리 삭제(부활 방지).
 */
