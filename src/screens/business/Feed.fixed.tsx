...
const BusinessFeedScreen: React.FC = () => {
  ...
  const pendingInitCameraRef = useRef(false);
  ...
  // ✅ mapReady 이후에만 init camera(100m) 1회 강제 적용
  useEffect(() => {
    if (loading) return;
    if (!mapReady) return;
    if (didInitCameraRef.current) return;
    if (!pendingInitCameraRef.current) return;

    didInitCameraRef.current = true;
    pendingInitCameraRef.current = false;

    // ✅ 시작 카메라는 "즉시(0ms)" 적용해서 불필요한 줌 튐/재애니메이션 방지
    setCameraByRegion(initialRegionRef.current, 0);
  }, [loading, mapReady, setCameraByRegion]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // ✅ Mapbox가 준비되기 전 setCamera가 씹히는 문제 방지:
        //    mapReady 이후 effect에서 1회 강제 적용
        pendingInitCameraRef.current = true;
