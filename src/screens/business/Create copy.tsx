...
const CATEGORY_MINOR_OPTIONS: Record<string, string[]> = {
  ...
  '문화/여가': ['PC방', '노래방', '당구장', '스포츠/레저', '기타 문화/여가'],
  쇼핑: ['편의점', '마트/슈퍼', '의류', '잡화', '생활/리빙', '기타 쇼핑'],
  서비스: ['헤어/미용', '네일/피부', '세탁', '수리/AS', '기타 서비스'],
  숙박: ['호텔', '모텔', '게스트하우스', '팬션/민박', '기타 숙박'],
  기타: ['기타 업종'],
};
...
const BusinessCreateScreen: React.FC = () => {
  ...
  const [aiBriefingUpdatedAt, setAiBriefingUpdatedAt] =
    useState<string | null>(null);
...
// 자동완성 결과 선택
  ...
  const loadBusinessAndRelated = useCallback(async () => {
    try {
      ...
      // 메뉴 + 메뉴 아이템
      {
        ...
        setMenus((mData ?? []) as BusinessMenu[]);
        ...
        const by: MenuItemsByMenuId = {};
        (iData ?? []).forEach((row: any) => {
          const mId = row.menu_id;
          if (!by[mId]) by[mId] = [];
          by[mId].push(row as BusinessMenuItem);
        });
        setMenuItemsByMenuId(by);
      }
      ...
      // 방문자 피드 count
      {
        const { count } = await supabase
          ...
          .eq('business_id', businessId);
        setVisitorFeedCount(count ?? null);
      }
    } catch (err: any) {
      console.error(err);
  ...
  useEffect(() => {
    ...
    setAiBriefing((p) => p || anyBiz.ai_briefing || '');
    setAiBriefingUpdatedAt(() => {
      if (!anyBiz.ai_briefing_updated_at) return null;
      return new Date(anyBiz.ai_briefing_updated_at).toLocaleString('ko-KR');
    }); 

    // 카테고리 / 19세 여부 초기값
    setCategoryMajor((p) => p || anyBiz.category_major || anyBiz.category || '');
...
const handleGenerateAiBriefing = useCallback(async () => {
  if (!business?.id) {
    Alert.alert(
      '안내',
      '먼저 가게 정보를 저장한 후 AI 브리핑을 생성할 수 있습니다.',
    );
    return;
  }
  ...
  try {
    ...
    if (error) {
      // 🔥 여기서 Edge Function에서 내려준 status / body까지 로그로 확인
      console.error('ai-briefing error', error);
      const anyErr = error as any;
      const ctx = anyErr.context;
      console.log('ai-briefing context', ctx);
    ...
    console.error('ai-briefing unexpected error', e);
    Alert.alert(
      '오류',
