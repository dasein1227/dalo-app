...
const INITIAL_VIEW_METERS = 100; // ✅ 최초 진입 카메라(데이터 반경 2km와 분리)
const INITIAL_CAMERA_ZOOM = 18.8; // ✅ 약 100m 체감 줌(기기/위도에 따라 약간 차이)
const STALE_REFRESH_MS = 12 * 60 * 60 * 1000;
const SELECT_FOCUS_METERS = 100;
const SELECT_CAMERA_ZOOM = 18.8;
...
const BusinessFeedScreen: React.FC = () => {
  ...
  // ✅ Mapbox 초기 로딩 타이밍 때문에 "init camera(100m)"가 씹히는 문제 방지:
  //    - mapReady 이후에만 100m 카메라를 1회 강제로 적용
  const [mapReady, setMapReady] = useState(false);
  const [locationReady, setLocationReady] = useState(false);

  const [cameraState, setCameraState] = useState<CameraState>(() => ({
    center: [SEOUL.longitude, SEOUL.latitude],
    zoom: regionToZoom(SEOUL),
  }));
  ...
  const setCameraByRegion = useCallback(
    (r: Region, duration = 300) => {
      const zoom = regionToZoom(r);
      setCameraState({ center: [r.longitude, r.latitude], zoom, duration });
    },
    [setCameraState],
  );
...
// ✅ radius setter (50m~2km 강제)
  ...
  // ✅ Mapbox 로딩/카메라 준비 타이밍 이슈 방지:
  //    mapReady + locationReady 이후 초기(100m) 카메라를 1회 확정 적용
  useEffect(() => {
    if (!mapReady) return;
    ...
    const r = initialRegionRef.current;
    setCameraState({ center: [r.longitude, r.latitude], zoom: INITIAL_CAMERA_ZOOM });
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // ✅ 초기 카메라/중심은 '현재 위치' 기준으로 확정 (lastKnown은 해외/오래된 위치로 튀는 케이스가 있음)
        ...
        // ✅ Camera는 props(state)로 선언형 제어 (초기엔 100m 줌으로 고정)
        setCameraState({ center: [center.longitude, center.latitude], zoom: INITIAL_CAMERA_ZOOM });
  ...
  useEffect(() => {
    ...
    showRadiusHint();
    applyClientFilters();
  }, [radiusMeters, setCameraByRegion, showRadiusHint, applyClientFilters]);
      ...
      <View style={[styles.card, selected && styles.cardSelected]}>
        <Pressable
          onPress={() => {
            ...
            setCameraState({ center: [item._lng, item._lat], zoom: SELECT_CAMERA_ZOOM, duration: 280 });
  ...
  const onPressBusinesses = useCallback(
    async (e: any) => {
      ...
      if (isCluster && Array.isArray(coords) && coords.length >= 2) {
        ...
        try {
          const cz = nextZoom != null ? Math.min(22, nextZoom + 0.8) : null;
          if (cz != null) {
            setCameraState({ center: [coords[0], coords[1]], zoom: cz, duration: 260 });
          }
        } catch {}
        return;
      }
      ...
      <View style={{ flex: 1 }}>
        {loading ? (
          ...
          >
            <Camera
              ref={cameraRef}
              centerCoordinate={cameraState.center as any}
              zoomLevel={cameraState.zoom as any}
              {...(cameraState.duration
                ? ({ animationMode: 'easeTo', animationDuration: cameraState.duration } as any)
                : {})}
            />
