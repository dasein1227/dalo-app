...
const BusinessFeedScreen: React.FC = () => {
  ...
  // ✅ A안: 기본은 distance(=fallback-only로 시작)
  ...
  // ✅ onRegionDidChange deprecated → onMapIdle로 교체
  ...
  // =========================================================
  // “기본상태는 fallback, 의도 발생 시 RPC” 분기 (A안: 기본 sortKey=distance)
  // =========================================================
  const fetchBusinesses = useCallback(async () => {
    ...
    const normalizeRows = (rawRows: any[]) => {
      ...
      return (rawRows ?? [])
        .map((b: any): BusinessDraw | null => {
          ...
          const dLat = lat as number;
          const dLng = lng as number;
    ...
    try {
      ...
      // ✅ 의도 기준(운영 안전): 검색어 or 카테고리 지정 or 정렬(distance 이외)
      // A안에서는 기본 sortKey가 distance라 기본상태에서 false가 된다.
      const shouldUseAI =
        t.length > 0 ||
      ...
      if (__DEV__) {
        console.log('[biz-search] shouldUseAI=', shouldUseAI);
      }

      // 1) 기본상태: fallback-only
      if (!shouldUseAI) {
        const raw = await fallbackFetch();
        const rows = sortRows(normalizeRows(raw));
        if (__DEV__) console.log('[biz-search] path=fallback-only rows=', rows.length);
        setBusinesses(rows);
        return;
      }

      // 2) 의도 발생: RPC (interpret는 검색어 있을 때만)
      const qParts: string[] = [];
      if (t) qParts.push(t);
      ...
      if (__DEV__) {
        console.log('[biz-search] query=', query);
        console.log('[biz-search] signals=', JSON.stringify(signals));
        console.log('[biz-search] categoryTop3=', JSON.stringify(categoryTop3));
      }
      ...
      try {
        ...
        if (__DEV__) {
          console.log('[biz-search] ranked.len=', Array.isArray(rawRows) ? rawRows.length : -1);
          console.log(
            '[biz-search] ranked.first=',
            rawRows?.[0]
              ? JSON.stringify({
                  business_id: rawRows[0].business_id,
                  score: rawRows[0].score,
                  explain: rawRows[0].explain,
                })
              : null,
          );
        }

        // 결과가 0이면 상용앱 방어: fallback로 최소 리스트는 보여줌
        ...
        usedFallback = true;
        if (__DEV__) console.log('[biz-search] rpcErr/fallback=', String(e?.message ?? e));
        rawRows = await fallbackFetch();
      }

      if (__DEV__) console.log('[biz-search] usedFallback=', usedFallback);
    ...
    <SafeAreaView style={styles.safeArea} edges={['left', 'right', 'bottom']}>
      ...
      <BusinessSearchModal
        ...
        onReset={() => {
          ...
          setHideAdult(false);
          setSearchText('');
          setSortKey('distance'); // ✅ A안: 기본은 distance
          setOpenNow(false);
        }}
        onApply={() => {
          debouncedFetchRef.current?.();
