...
type BusinessRow = {
  ...
  // ✅ 둘 다 받되, “지금 영업중”은 is_open_now를 사용
...
const BusinessFeedScreen: React.FC = () => {
  ...
  // ✅ 추가: 정렬 + 지금 영업중
  const [sortKey, setSortKey] = useState<BusinessSortKey>('recommended');
  ...
  const handleMapRegionDidChange = useCallback((e: any) => {
    const coords =
  ...
  // =========================================================
  // RPC 연결 버전 (UI는 그대로, fetch 로직만 교체)
  // =========================================================
  const fetchBusinesses = useCallback(async () => {
    ...
    try {
      ...
      let signals: any[] = [];
      let categoryTop3: string[] = selectedCategory !== 'all' ? [String(selectedCategory)] : [];
      ...
      const rows = (rawRows ?? [])
        .map((b: any): BusinessDraw | null => {
          ...
          return {
            ...
            ...(b?.explain ? { explain: b.explain } : null),
            ...(b?.ai_category_top3 ? { ai_category_top3: b.ai_category_top3 } : null),
            ...(b?.profile_updated_at ? { profile_updated_at: b.profile_updated_at } : null),
            ...(b?.score != null ? { score: scoreNum } : null),
        ...
        .filter((b) => {
          ...
          const catOk = selectedCategory === 'all' ? true : (b.category || null) === selectedCategory;
          const eventOk = onlyWithEvent ? !!b.has_active_event : true;
          const adultOk = hideAdult ? !b.is_adult : true;

          // ✅ “지금 영업중”은 is_open_now로 필터
  ...
  const renderItem = ({ item }: { item: BusinessDraw }) => {
    ...
    const ratingText =
      item.rating && item.review_count ? `★ ${item.rating.toFixed(1)} · 리뷰 ${item.review_count}` : '리뷰 없음';
    ...
    return (
      <View style={[styles.card, selected && styles.cardSelected]}>
        ...
        <Pressable onPress={() => navigation.navigate('BusinessDetail', { businessId: item.id })} style={styles.selectBtn}>
      ...
      <View style={[styles.headerContainer, { paddingTop: HEADER_TOP_PAD }]}>
        <View style={styles.headerRow}>
          ...
          <View style={styles.headerIconsRow}>
            ...
            <Pressable style={styles.headerIconButton} onPress={() => rootNav.navigate('SettingsHome')}>
      ...
      <View style={{ flex: 1 }}>
        {loading ? (
          ...
          <MapView
            ...
            onRegionDidChange={handleMapRegionDidChange as any}
            logoEnabled={false}
          ...
          >
            <Camera
              ref={cameraRef}
              defaultSettings={{
                centerCoordinate: [initialRegionRef.current.longitude, initialRegionRef.current.latitude],
