      ...
      <View style={[styles.stepRow, { gap }]}>
        <Pressable onPress={dec} hitSlop={8} style={styles.stepBtn} disabled={atMin}>
          <Text
            style={[
              styles.stepBtnTxt,
              { fontSize: arrowFS, opacity: atMin ? 0.3 : 1 },
            ]}
          >
            {'<'}
          </Text>
        ...
        <Pressable onPress={inc} hitSlop={8} style={styles.stepBtn} disabled={atMax}>
          <Text
            style={[
              styles.stepBtnTxt,
              { fontSize: arrowFS, opacity: atMax ? 0.3 : 1 },
            ]}
          >
            {'>'}
          </Text>
...
export default function CreateBeacon({ navigation }: any) {
  ...
  // --- 기존 활성 비활성화 (host_id 소유의 예전 active 비콘 끄기)
const closeActiveBeacons = useCallback(async (hostId: string) => {
...
const createBeacon = async () => {
  ...
  try {
    ...
    // 8) 기존 활성 비콘 비활성화
    await closeActiveBeacons(user.id);

    // 9) insert payload
    const payload: any = {
      host_id: user.id,
      title: (status ?? '').trim() || '(제목 없음)',
      ...
      allow_gender: visibleGender ?? 'any',
      require_approval: requireApproval,

      // 인원/제한
      male_quota: nz(maleQuota),
      female_quota: nz(femaleQuota),
      mix_quota: nz(mixQuota),
      max_members: nz(maleQuota + femaleQuota + mixQuota),
    ...
    const newBeaconId = inserted?.id;

    // 11) 초대
    if (newBeaconId && invitees.length > 0) {
      try {
    ...
    // 12) 라벨 공유
    if (newBeaconId && visibility === 'labels' && selectedLabelIds.length > 0) {
      try {
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: '#fff' }}>
          ...
          <View style={styles.panel}>
            ...
            <View style={styles.cardBlock}>
              ...
              <View
                style={[
                  styles.stepperRow,
                  { columnGap: Math.round(8 * sizeFactor) },
                ]}
              >
                <Stepper
      ...
      >
        ...
        <View style={styles.bottomSheet}>
          {(
            ['friends', 'labels', 'public_filtered', 'custom'] as Visibility[]
          ).map((v) => (
            <Pressable
              key={v}
              style={[
      ...
      >
        ...
        <View style={styles.bottomSheet}>
          {(['public', 'private'] as const).map((mode) => {
            const on =
              (profilePublic && mode === 'public') ||
              (!profilePublic && mode === 'private');
            return (
              <Pressable
                key={mode}
                style={[
                  styles.sheetRow,
                  on && styles.sheetRowActive,
                ]}
                onPress={() => {
        ...
        <View style={styles.bottomSheet}>
          {(['any', 'male', 'female'] as VisibleGender[]).map((g) => (
            <Pressable
              ...
              style={[
                styles.sheetRow,
                visibleGender === g && styles.sheetRowActive,
              ]}
              onPress={() => {
                setVisibleGender(g);
                setSheet(null);
      ...
      >
        ...
        <View style={styles.sheet}>
          ...
          <FlatList
            ...
            ListEmptyComponent={
              <Text style={styles.emptyTxt}>친구가 없습니다.</Text>
            }
          />
