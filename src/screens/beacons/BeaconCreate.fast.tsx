      ...
      if (!roomId) {
        const { data: createdRoom, error: insErr } = await supabase
          ...
          .insert({
            ...
            custom_title: title || '(제목 없음)',
            created_by: user.id,
          })
          .select('id')
          .single();
        if (!insErr && createdRoom?.id) roomId = createdRoom.id;
      }
        ...
        if (mountedRef.current) {
          try {
            rootNav.navigate('Chat', {
              ...
              beaconTitle: title || '(제목 없음)',
              customTitle: title || '(제목 없음)',
            });
          } catch {}
        }
      }
    } catch {}
  }, [invitees, visibility, selectedLabelIds, rootNav]);
