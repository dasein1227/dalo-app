...
export default function ChatRoomsScreen() {
  ...
  // =========================
  // ✅ StatusBar: 투명 + 헤더가 StatusBar 영역까지 확장
  // =========================
  const applyStatusBar = React.useCallback(() => {
    try {
      // native-stack 옵션이 있는 경우 대응 (없으면 무시)
  ...
  const loadFriends = React.useCallback(async () => {
    try {
      ...
      const { data: pf, error: pfErr } = await supabase.from('profiles').select('id, nickname, avatar_url').in('id', ids as any);
  ...
  const fetchBeaconList = React.useCallback(async (): Promise<Row[]> => {
    ...
    const joinedRoomIds = Array.from(new Set((memberRooms ?? []).map((r: any) => r.room_id as number).filter(Boolean)));
    ...
    if (ownerIds.length) {
      const { data: pf, error: pfErr } = await supabase.from('profiles').select('id, nickname, avatar_url').in('id', ownerIds as any);
    ...
    if (allBeaconIds.length) {
      ...
      const roomIds = Array.from(new Set(roomsForBeacons.map((r) => r.id as number | null).filter((v): v is number => v !== null)));
      ...
      if (roomIds.length) {
        ...
        const roomLastMap = new Map<number, { content: string | null; created_at: string | null }>();
  ...
  const fetchBusinessDMList = React.useCallback(async (): Promise<Row[]> => {
    ...
    const joinedIds = Array.from(new Set((joinedRooms ?? []).map((r: any) => r.room_id as number).filter((v) => v != null)));
    if (!joinedIds.length) return [];

    // 2) business_dm 인 dm 방만
    const { data: roomData, error: roomErr } = await supabase
      .from('chat_rooms')
      .select('id, type, subtype, custom_title, beacon_id, created_at, created_by')
      .in('id', joinedIds as any)
      .eq('type', 'dm')
      .eq('subtype', 'business_dm');

    if (roomErr) {
      console.log('chat_rooms (business_dm) error', roomErr);
    }
    ...
    const lastMap = new Map<number, { content: string | null; created_at: string | null }>();
    for (const m of (msgData ?? []) as any[]) {
      const rid = m.room_id as number;
      if (!lastMap.has(rid)) {
        lastMap.set(rid, {
    ...
    // 4) DM 상대방(피어) 추출: room_id -> peer_user_id
    ...
    if (memErr) {
      console.log('chat_members (business_dm) error', memErr);
    }

    const membersByRoom = new Map<number, string[]>();
    for (const mr of (memRows ?? []) as any[]) {
      const rid = mr.room_id as number;
    ...
    const peerIdByRoom = new Map<number, string>();
    for (const r of rooms) {
      const rid = r.id as number;
      if (!rid) continue;
      const mems = membersByRoom.get(rid) ?? [];
      const peer = mems.find((uid) => uid && uid !== userId) ?? null;
      if (peer) peerIdByRoom.set(rid, peer);
    }

    const ownerIds = Array.from(new Set(rooms.map((r) => r.created_by as string).filter(Boolean)));
    const peerIds = Array.from(new Set(Array.from(peerIdByRoom.values()).filter((id) => id && id !== userId)));

    // 5) 프로필(방장 + DM 상대 + 나)
    const profileIds = Array.from(new Set([userId, ...ownerIds, ...peerIds].filter(Boolean)));
    let profiles: Record<string, { nickname?: string | null; avatar_url?: string | null }> = {};
    if (profileIds.length) {
      ...
      if (pfErr) {
        console.log('profiles (business_dm) error', pfErr);
      }
    ...
    const mapped: Row[] = rooms.map((r) => {
      ...
      const peerId = peerIdByRoom.get(r.id as number) ?? null;
      const peerNick = peerId ? profiles[peerId]?.nickname ?? null : null;
      const peerAvatar = peerId ? profiles[peerId]?.avatar_url ?? null : null;

      const displayTitle = r.custom_title || peerNick || '비즈니스 채팅';
      const displayAvatar = peerAvatar || profiles[r.created_by]?.avatar_url || null;
  ...
  const fetchNormalList = React.useCallback(
    async (modeKey: Exclude<ChatListMode, 'beacon'>): Promise<Row[]> => {
      ...
      // 2) 실제 방 정보
      let roomQuery = supabase.from('chat_rooms').select('id, type, subtype, custom_title, beacon_id, created_at, created_by').in('id', joinedIds as any);

      if (modeKey === 'personal') {
        // personal: dm + self (단 business_dm은 여기서 제외)
      ...
      let rooms = (roomData ?? []) as any[];
      if (!rooms.length) return [];

      if (modeKey === 'personal') {
        // ✅ 요구사항: business_dm은 personal에서 절대 안 보이게
        rooms = rooms.filter((r) => {
          if (r.type === 'self') return true;
          if (r.type === 'dm' && r.subtype === 'business_dm') return false;
          return r.type === 'dm';
        });
      }
      ...
      if (memErr) {
        console.log('chat_members (list) error', memErr);
      }
      ...
      for (const r of rooms) {
        ...
        // self 방이면 굳이 peer가 없지만, 프로필/아바타 계산을 위해 내 id를 넣어둔다
        if (r.type === 'self') {
          peerIdByRoom.set(rid, userId);
        }
      }
      ...
      // 6) 표시용 매핑
      const mapped: Row[] = rooms.map((r) => {
        const isRoomCreator = r.created_by === userId;
        const isOwner = r.type === 'open' ? isRoomCreator : false;
        const last = lastMap.get(r.id as number);
        const updated = last?.created_at ?? r.created_at ?? null;

        // 진짜 self 방인지 판정: type = 'self' + 내가 만든 방 + beacon_id 없음
        const isSelfChat = r.type === 'self' && isOwner && !r.beacon_id;
        ...
        let displayTitle =
          r.type === 'group'
            ? r.custom_title || '그룹 채팅'
            : r.type === 'open'
              ? r.custom_title || '오픈 채팅'
              : r.custom_title || ownerNick || '채팅방';
        let displayAvatar = profiles[r.created_by]?.avatar_url || null;

        if (isSelfChat) {
          displayTitle = '나와의 채팅';
          displayAvatar = profiles[userId]?.avatar_url || displayAvatar;
          } else if (r.type === 'dm') {
            // DM은 '상대'가 기본. created_by(=ownerNick)로 타이틀을 절대 대체하지 않는다.
            displayTitle = r.custom_title || peerNick || '1:1 채팅';
            displayAvatar = peerAvatar || displayAvatar;

            // peer를 못 구한 경우는 서버/RLS/데이터 문제이므로 로그를 남긴다.
            if (!peerId || peerId === userId) {
              console.log('[chat-list] DM peer missing', {
                roomId: r.id,
                created_by: r.created_by,
                members: membersByRoom.get(r.id as number) ?? [],
              });
            }
          }
  ...
  const fillUnreadCounts = React.useCallback(async (list: Row[]) => {
    ...
    const roomIds = Array.from(new Set(list.map((r) => Number(r.id)).filter((v) => Number.isFinite(v))));
    if (roomIds.length === 0) return list;

    try {
      const { data, error } = await supabase.rpc('get_my_unreads', { p_room_ids: roomIds });
      if (error) {
    ...
    try {
      ...
      const { data: room, error: roomErr } = await supabase
        ...
        .insert({
          ...
          beacon_id: null,
          created_by: me,
          // ✅ 일반 생성은 subtype null (business는 별도 흐름에서 생성/연결)
          subtype: null,
        })
        .select('id')
        .single();
  ...
  const ModeTabs = () => {
    ...
    const handlePressTab = (key: ChatListMode) => {
      if (key !== 'beacon') {
        // 다른 탭으로 가면 비콘 토글은 기본값으로 복귀(원하면 이 줄 삭제 가능)
        if (mode === 'beacon') setBeaconMode('beacon');
        setMode(key);
        return;
      }

      // ✅ 비콘 탭: 비활성 -> 비콘으로 이동, 활성 -> 비콘/비즈니스 토글
      if (mode !== 'beacon') {
        setMode('beacon');
        return;
      }
        ...
        <View style={styles.modeTabsWrapper}>
          <View style={styles.modeTabsBg}>
            {tabs.map((t) => {
              ...
              // ✅ 비콘 탭이 active일 때 라벨을 상태에 따라 변경 (비콘/비즈니스)
  ...
  // ---------- 즐겨찾기 / 일반 채팅방 분리 ----------
  const favoriteRows = React.useMemo(() => rows.filter((r) => r.favorite && !specialIdSet.has(String(r.id))), [rows, specialIdSet]);
        ...
        <View style={styles.newChatBackdrop}>
          <View style={styles.newChatBox}>
            ...
            <View style={{ marginTop: 4, marginBottom: 6 }}>
              <Text style={styles.newChatLabel}>{newChatType === 'personal' ? '대화할 친구를 선택하세요' : '초대할 친구들을 선택하세요'}</Text>
            ...
            <View style={styles.newChatButtonsRow}>
              ...
              <Pressable style={[styles.newChatButton, creatingChat && { opacity: 0.7 }]} onPress={handleCreateChat} disabled={creatingChat}>
  ...
  // ---------- 메인 렌더 ----------
  return (
    // ✅ top safe-area는 우리가 직접(headerContainer paddingTop=insets.top) 처리하므로 edges에서 top 제거
...
const styles = StyleSheet.create({
  ...
  headerContainer: {
    ...
    // ✅ paddingTop은 컴포넌트에서 insets.top + 4로 주입
    paddingBottom: 8,
  },
