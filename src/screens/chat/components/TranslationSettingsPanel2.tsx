...
export const TRANSLATION_LANGUAGE_OPTIONS: { code: string; label: string }[] = [
  { code: 'EN', label: 'EN' },
  { code: 'ES', label: 'ES' },
  { code: 'FR', label: 'FR' },
  { code: 'DE', label: 'DE' },
...
function normalizeLang(v: string | null | undefined) {
  ...
  return up === AUTO_SEND_LANG ? null : up;
}
...
export function labelByTranslationLang(code: string | null | undefined, t?: AppTranslateFn) {
  ...
  const fallback = hit ? hit.label : normalized;
  return t ? panelText(t, `language.${normalized}`) : fallback;
}
