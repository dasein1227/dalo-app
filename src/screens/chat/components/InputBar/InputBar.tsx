...
type Props = {
  ...
  /**
   * ✅ "번역문만 보기" 모드일 때,
   * 답장 프리뷰도 번역문이면 번역문, 원문 모드면 원문을 보여주기 위한 플래그
   */
...
export default function InputBar({
  ...
  const [defaultPanelH, setDefaultPanelH] = useState<number>(FALLBACK_PANEL_H);
  ...
  React.useEffect(() => {
    ...
    (async () => {
      ...
      const cached = await getCachedKeyboardHeight(key);

      if (!alive) return;

      if (cached != null) {
        const clamped = Math.max(PANEL_MIN_H, Math.min(420, cached));
        setDefaultPanelH(clamped);
      } else {
        setDefaultPanelH(FALLBACK_PANEL_H);
      }
    })();
  ...
  const persistKeyboardHeight = useCallback(async (measured: number) => {
    const key = cacheKeyRef.current;
    const clampedNow = Math.max(PANEL_MIN_H, Math.min(420, Math.round(measured)));
  ...
  const onSend = useCallback(async () => {
    ...
    // 1. 전송할 데이터를 미리 변수에 저장 (상태가 비워지기 전 캡처)
    const messageContent = trimmed;
    const messageReplyTo = replyTo;

    // 2. UI 상태를 '먼저' 즉시 초기화 (사용자는 전송되었다고 느낌)
    setText('');
    cancelReply();

    // 3. 서버 전송은 비동기로 처리 (await를 유지하되 UI에 영향을 주지 않음)
    try {
      await sendMessage({
      ...
      // (선택사항) 전송 실패 시 에러 처리 또는 텍스트 복구 로직
  ...
  const pillBg = theme.inputFieldBg; // ✅ 입력필드 박스 배경
  const textColor = theme.opponentText;
  const placeholderColor = 'rgba(0,0,0,0.35)';

  const attachmentIconColor = theme.accessoryIcon; // ✅ 첨부 아이콘들
  const voiceIconColor = '#ffffff'; // 음성 버튼은 솔리드 배경이므로 흰색
  const voiceBg = theme.voiceButton; // ✅ voiceButton 사용
  const sendBg = theme.sendButtonActive; // ✅ sendButtonActive 사용
  const panelBg = theme.inputBg;

  /**
   * ✅ 답장 프리뷰에서 "원문/번역문" 선택
   * - replyTo.contentOriginal / contentTranslated (Chat.tsx에서 심어둔 값)
   * - InputReplyPreview를 건드리지 않기 위해, replyTo.content만 안전하게 덮어쓴 "프록시"를 전달
   */
  const replyToForPreview = useMemo<ReplyInfo | null>(() => {
    ...
    // ReplyInfo 타입을 유지하면서 content만 교체
  ...
  return (
    <Animated.View style={[styles.root, { backgroundColor: rootBg }, rootStyle]}>
      {/* ✅ reply 프리뷰: 원문/번역문 모드에 맞게 content를 덮어쓴 replyToForPreview 전달 */}
