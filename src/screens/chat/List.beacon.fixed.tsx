...
type Row = ChatRoomRow & {
  ...
  // ✅ ChatRoomCard 호환(혹시 unread_count를 쓰는 구현 대비)
...
export default function ChatRoomsScreen() {
  ...
  const fillUnreadCounts = React.useCallback(async (list: Row[]) => {
    ...
    // ✅ bigint[] RPC에 안전하게: number로 캐스팅해서 보냄
    const rawIds = Array.from(new Set(list.map((r) => r.id).filter((v: any) => v != null)));
    const roomIdsNum = rawIds
      .map((v) => toFiniteInt(v))
      .filter((v): v is number => typeof v === 'number' && Number.isFinite(v));

    if (roomIdsNum.length === 0) return list;

    try {
      // 1차: { p_room_ids }
      let { data, error } = await supabase.rpc('get_my_unreads', { p_room_ids: roomIdsNum });

      // 2차 fallback: { room_ids } (파라미터명이 다른 경우)
      if (error) {
        dbg('get_my_unreads error (p_room_ids)', error);
        const res2 = await supabase.rpc('get_my_unreads', { room_ids: roomIdsNum } as any);
        data = res2.data;
        error = res2.error;
      }

      // 3차 fallback: 혹시라도 캐스팅 문제면 rawIds(문자열)로 한 번 더
      if (error) {
        dbg('get_my_unreads error (room_ids)', error);
        const res3 = await supabase.rpc('get_my_unreads', { p_room_ids: rawIds } as any);
        if (!res3.error) {
          data = res3.data;
          error = null;
        }
      ...
      if (error) {
        dbg('get_my_unreads error (final)', error);
        return list;
      }
      ...
      for (const u of (data ?? []) as any[]) {
        const rid = u?.room_id;
        const cntRaw = u?.unread_count ?? u?.unread ?? u?.unreadCount ?? 0;
        const cnt = Number(cntRaw);
        unreadMap.set(String(rid), Number.isFinite(cnt) ? cnt : 0);
      }

      return list.map((r) => {
        const cnt = unreadMap.has(String(r.id)) ? unreadMap.get(String(r.id))! : 0;
        return {
          ...r,
          unread: cnt,
          unread_count: cnt,
    ...
    try {
      ...
      if (mode === 'beacon') {
        if (beaconMode === 'beacon') {
          ...
          setRows(baseList); // 비콘 목록은 (beacon_id 기반) unread 계산 구조가 달라서 여기선 유지
        } else {
    ...
    (row: Row): void => {
      (async () => {
        ...
        try {
          ...
          // ✅ beacon은 하드코딩: 비콘 진입 경로에서는 무조건 beacon 타입으로 Chat에 전달
const isBeaconRoom =
  mode === 'beacon' ||
  beaconMode === 'beacon' ||
  (anyRow as any)?.beacon_id != null ||
  (anyRow as any)?.beaconId != null;
...
if (isBeaconRoom) {
  nav.navigate('Chat', {
    ...
    beaconId:
      (anyRow as any)?.beacon_id != null
        ? String((anyRow as any).beacon_id)
        : (anyRow as any)?.beaconId != null
          ? String((anyRow as any).beaconId)
          : null,
    beaconTitle:
      typeof (anyRow as any)?.custom_title === 'string' && (anyRow as any).custom_title.trim()
        ? (anyRow as any).custom_title.trim()
        : typeof (anyRow as any)?.title === 'string' && (anyRow as any).title.trim()
          ? (anyRow as any).title.trim()
          : null,
  });
  return;
}
