...
type ProfLite = { nickname?: string | null; avatar_url?: string | null };
  ...
  useEffect(() => {
    ...
    if (mounted && m) {
      ...
      const ids = Array.from(
        new Set(((mems as Member[]) ?? []).map((x) => x.user_id))
      );
    ...
    (async () => {
      ...
      if (roomRow?.type === 'dm') {
        ...
        if (otherIds.length === 1) {
          const other = profiles[otherIds[0]];
          resolvedTitle = other?.nickname?.trim() || '대화상대';
        }
      } else if (roomRow?.type === 'group') {
        if (roomRow.custom_title?.trim()) {
          resolvedTitle = roomRow.custom_title.trim();
        } else {
          const nicks = members
            .map((m) => profiles[m.user_id]?.nickname || '')
            .filter((s) => !!s.trim());
          resolvedTitle =
      ...
      } else if (roomRow?.type === 'open') {
        resolvedTitle = roomRow.custom_title?.trim()
          ? roomRow.custom_title.trim()
          : `오픈채팅 #${roomPk}`;
      } else if (roomRow?.type === 'beacon') {
        if (roomRow?.beacon_id != null) {
          ...
          try {
            const { data: vrow } = await supabase
              ...
              .eq('id', beaconPk)
              .maybeSingle();
            if (vrow?.title?.trim()) fetchedTitle = vrow.title.trim();
          } catch {}
          ...
          if (!fetchedTitle) {
            try {
              const { data: brow } = await supabase
                ...
                .eq('id', beaconPk)
                .maybeSingle();
              if (brow?.title?.trim()) fetchedTitle = brow.title.trim();
            } catch {}
          }

          if (fetchedTitle) resolvedTitle = fetchedTitle;
        }
      ...
      if (!resolvedTitle) resolvedTitle = `채팅방 #${roomPk}`;
      if (mounted) {
      ...
      if (roomRow?.type === 'beacon' && roomRow?.beacon_id) {
        const beaconIdNum = Number(roomRow.beacon_id);
        const ch = supabase
          .channel(`beacon_title_${beaconIdNum}`)
          .on(
            'postgres_changes',
            {
              event: 'UPDATE',
              schema: 'public',
              table: 'beacons',
              filter: `id=eq.${beaconIdNum}`,
            },
            (payload) => {
              const nt = (payload.new as any)?.title;
              if (typeof nt === 'string' && nt.trim()) setTitle(nt.trim());
            }
          )
          .subscribe();
        unsub = ch;
        ...
        onSendSelected={async (assets, bundleSend) => {
          try {
            for (const a of assets) {
              ...
              const tag = a.isVideo ? '[video]' : '[image]';
              await send(`${tag}${publicUrl}|${a.filename}`);
            }
          } catch (e: any) {
            Alert.alert('업로드 실패', e.message ?? String(e));
          }
