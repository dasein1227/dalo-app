...
export type ClusteredMapProps = {
  ...
  /**
   * 처음 로딩시 "외국에서 날아오는" 현상 방지:
   * - center/zoom 준비 전 MapView 자체를 렌더하지 않으면 0,0 화면이 안 나옴
   */
  renderWhenCameraReady?: boolean;

  /** 초기 카메라 애니메이션 (기본 0: 날아오는 연출 제거) */
   ...
   * - pointIconName/clusterIconName에 key를 지정
   */
  ...
  /** cluster icon key (when point_count exists) */
...
const ClusteredMap: React.FC<ClusteredMapProps> = ({
  ...
  if (renderWhenCameraReady && !cameraReady) {
    // MapView 자체를 렌더하지 않음: 0,0(외국) 화면 방지
  ...
  const clusterIcon = clusterIconName || baseIcon;
        ...
        <>
          ...
          <MapboxGL.SymbolLayer
            ...
            style={{
              ...
              iconSize: ['step', ['get', 'point_count'], 0.9, 10, 1.0, 30, 1.1, 70, 1.2],
            }}
          />
          ...
          <MapboxGL.SymbolLayer
            ...
            style={{
              ...
              iconSize: ['case', ['==', ['get', 'highlighted'], 1], 1.28, ['==', ['get', 'selected'], 1], 1.14, 1.0],
              iconAllowOverlap: true,
              iconIgnorePlacement: true,
            }}
          />

          {/* optional emoji label for business */}
          {variant === 'business' ? (
            <MapboxGL.SymbolLayer
